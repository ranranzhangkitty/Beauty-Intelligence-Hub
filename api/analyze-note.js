export default async function handler(req,res){
  ...
  try{
    ...
    const aiRes=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENROUTER_API_KEY}`,'Content-Type':'application/json','HTTP-Referer':'https://ranran-beauty-hub.vercel.app','X-Title':'New Finds'},body:JSON.stringify({model:'openrouter/free',response_format:{type:'json_object'},messages:[{role:'user',content:[{type:'text',text:prompt},...signed.map(x=>({type:'image_url',image_url:{url:x}}))]}]})});
    if(!aiRes.ok)throw Error('OpenRouter rejected the request: '+await aiRes.text());
    const ai=await aiRes.json(),result=JSON.parse(ai.choices[0].message.content.replace(/^```json|```$/g,'').trim()),fields=['category','title','brand','source','price','launch_date','market','core_highlight','core_benefit','consumer_insight','ocr_text'];
